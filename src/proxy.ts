...
const publicRoutes = [
  ...
  FRONTEND_ROUTES.LANDING_PAGE_ONE,
  FRONTEND_ROUTES.LANDING_PAGE_TWO,
  FRONTEND_ROUTES.LANDING_PAGE_THREE,
  FRONTEND_ROUTES.LANDING_PAGE_FOUR,
  "/landing-five"
];
const authRoutes = [
  FRONTEND_ROUTES.LOGIN,
...
function getUserFromCookie(request: NextRequest): User | null {
  try {
    const userCookie = request.cookies.get(AUTH_CONFIG.userKey)?.value;
...
export const config = {
  matcher: [
    '/((?!api|_next/static|_next/image|favicon.ico|.*\\.(?:svg|png|jpg|jpeg|gif|webp|ico|woff2|woff|ttf|otf)).*)',
  ],
};
