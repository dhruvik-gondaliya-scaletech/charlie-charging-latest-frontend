...
export function GlobalPresenceSection() {
  ...
  const stats = [
    ...
    { value: 25, suffix: 'M+', label: 'Monthly Sessions', iconName: 'Activity' },
  ...
  return (
    <section id="global" className="py-24 bg-white relative overflow-hidden">
      ...
      <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[700px] h-[700px] bg-blue-500/5 rounded-full blur-[150px] pointer-events-none -z-10" />
      ...
      <div className="max-w-7xl mx-auto px-6">
        ...
        <div className="relative w-full aspect-[2/1] rounded-3xl border border-gray-200 bg-gray-50/50 overflow-hidden mb-16 p-4 shadow-inner">
          <svg
          ...
          {mapPins.map((pin) => (
            ...
            >
              {/* Outer pulsing ring */}
              <span className="absolute -left-1.5 -top-1.5 w-4 h-4 rounded-full bg-blue-500/40 animate-ping pointer-events-none" />
              {/* Inner core */}
              <span className="absolute -left-1 -top-1 w-3 h-3 rounded-full bg-blue-600 border-2 border-white shadow-md cursor-pointer" />
              
              {/* Label details */}
              <span className="absolute left-4 -top-3 scale-0 group-hover:scale-100 bg-gray-900 text-white text-[9px] font-bold px-2 py-1 rounded shadow-md transition-all whitespace-nowrap z-20 pointer-events-none uppercase tracking-wider">
                {pin.name}
              </span>
            </div>
          ))}
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-8">
          {stats.map((stat) => {
            ...
            return (
              <div
                key={stat.label}
                className="flex flex-col space-y-2 border-l border-gray-200 pl-6 group"
              >
